...
/* on pre comment, incre by one. If comment_mismatch_num > 0 and met post comment, decre by one.
 if comment_mismatch_num = 0 and met post comment, error: unmatched *)
 if comment_mismatch_num > 0 and EOF met, error: EOF in comment
*/
int comment_mismatch_num = 0;
...
"(*" {
  BEGIN(COMMENT);
  comment_mismatch_num = 1;
}
<COMMENT>"(*" {
  comment_mismatch_num++;
}
<COMMENT>"\n" {curr_lineno++;}
<COMMENT>"*)" {
  if (comment_mismatch_num == 0) {
